...
public class RedisService {
    ...
    public Long getExpire(String key, TimeUnit unit) {
        if(null==key||null==unit){
            throw new IllegalArgumentException("key or TomeUnit 不能为空");
        }
    ...
    public Boolean setifAbsen(String key,Object value,long time,TimeUnit unit){
        if(null==key||null==value||null==unit){
            throw new IllegalArgumentException("kkey、value、unit都不能为空");
        }
...
//******************hash数据类型*********************
    ...
    public Long hIncrBy(String key,Object field,long increment){
        if (null==key||null==field){
            throw new IllegalArgumentException("key or field 不能为空");
        }
    ...
    /**
     * 返回指定Key关联的链表中元素的数量，如果该Key不存在，则返回0。如果与该Key关联的Value的类型不是链表，则抛出相关的异
     常。
     * @Author:     ds
     * @UpdateUser:
     * @Version:     1.0
